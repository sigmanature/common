...
#define MTHP_COW_ORDER2_PARAM "/sys/module/kernel/parameters/mthp_cow_order2"

static int mthp_cow_order2_read_param(char *value)
{
	char buf[8];
	ssize_t ret;
	int fd;

	fd = open(MTHP_COW_ORDER2_PARAM, O_RDONLY);
	if (fd < 0)
		return -errno;
...
static int mthp_cow_order2_write_param(char value)
{
	ssize_t ret;
	int fd;

	fd = open(MTHP_COW_ORDER2_PARAM, O_WRONLY);
	if (fd < 0)
		return -errno;
...
static int child_mthp_cow_order2(char *mem, size_t size)
{
	const size_t nr_pages = size / pagesize;
	const size_t fault_idx = nr_pages / 2;
	...
	memset(mem + fault_idx * pagesize, 0x5a, pagesize);
	for (i = 0; i < nr_pages; i++) {
		unsigned long pfn;

		if (!pagemap_is_populated(fd, mem + i * pagesize)) {
			ret = KSFT_FAIL;
			goto out;
		}

		pfn = pagemap_get_pfn(fd, mem + i * pagesize);
		if (pfn == -1ul || !pfn) {
			ret = KSFT_SKIP;
			goto out;
		}
	...
	for (i = 0; i < nr_pages; i++) {
		char expected = i == fault_idx ? 0x5a : 0x10 + i;

		if (mem[i * pagesize] != expected) {
			ret = KSFT_FAIL;
			goto out;
		}
	}
...
static void run_mthp_cow_order2_test_case(void)
{
	struct thp_settings settings;
	char old_param = 'N';
	char *mem, *mmap_mem = MAP_FAILED;
	size_t size = pagesize << 2;
	size_t mmap_size = 2 * size;
	bool pushed_settings = false;
	bool restore_param = false;
	int order, ret, status;
	size_t i;

	log_test_start("mTHP order-2 COW copies a full PTE window");

	if (!pmdsize || size >= pmdsize) {
		log_test_result(KSFT_SKIP);
		return;
	}
	...
	ret = mthp_cow_order2_read_param(&old_param);
	if (ret) {
		log_test_result(KSFT_SKIP);
		return;
	}
	restore_param = true;
	if (old_param != 'Y' && old_param != '1') {
		ret = mthp_cow_order2_write_param('Y');
		if (ret) {
			log_test_result(KSFT_SKIP);
			return;
		}
	...
	order = sz2ord(size, pagesize);
	settings.hugepages[order].enabled = THP_ALWAYS;
	thp_push_settings(&settings);
	pushed_settings = true;
	...
	if (!pagemap_is_populated(pagemap_fd, mem + size - pagesize)) {
		ksft_print_msg("Did not get an order-2 mTHP populated\n");
		log_test_result(KSFT_SKIP);
		goto out;
	}
	for (i = 0; i < size / pagesize; i++)
		memset(mem + i * pagesize, 0x10 + i, pagesize);
	...
	if (ret < 0) {
		...
		exit(child_mthp_cow_order2(mem, size));
	}
...
out:
	...
	if (pushed_settings)
		thp_pop_settings();
	if (restore_param && old_param != 'Y' && old_param != '1')
		mthp_cow_order2_write_param(old_param);
}
