...
static void f2fs_finish_read_bio(struct bio *bio, bool in_task)
{
	...
	bio_for_each_folio_all(fi, bio) {
		...
		if (!folio_test_large(folio) &&
		    f2fs_is_compressed_folio(folio)) {
...
static void f2fs_write_end_io(struct bio *bio)
{
	...
	bio_for_each_folio_all(fi, bio) {
		...
		if (folio_has_ffs(folio)) {
			ffs = (struct f2fs_folio_state *)folio->private;
			finished = atomic_sub_and_test(nr_pages, &ffs->write_pages_pending);
		}
...
alloc_new:
	...
	io->last_block_in_bio = fio->new_blkaddr + (fio->cnt ? fio->cnt - 1 : 0);
				  ...
				  blk_opf_t op_flags, bool for_write)
{
	...
	bio = f2fs_grab_read_bio(inode, blkaddr, 1, op_flags, index,
				 for_write);
...
got_it:
	...
	f2fs_submit_page_read(inode, folio, index, dn.data_blkaddr,
							op_flags, for_write);
	return folio;
...
static inline unsigned int ffs_next_dirty_subpage(
			struct f2fs_folio_state *ffs, const struct folio *folio,
...
static inline unsigned int ffs_next_clean_subpage(
			struct f2fs_folio_state *ffs, const struct folio *folio,
			unsigned int start, unsigned int end)
{
	unsigned int nr_subpages = ffs_nr_subpages(folio);
				...
				u64 *range_start, u64 range_end)
{
	...
	if (start == end)
		...
		nr_pages = ffs_next_clean_subpage(ffs, folio, start + 1, end) - start;

	*range_start = folio_pos(folio) + ((u64)start << PAGE_SHIFT);
	return (u64)nr_pages << PAGE_SHIFT;
}

void ffs_mark_subrange_dirty(struct folio *folio, size_t offset,
			     size_t len)
{
	struct f2fs_folio_state *ffs;
	unsigned int nr_subpages = ffs_nr_subpages(folio);
...
void ffs_clear_subrange_dirty(struct folio *folio, size_t offset,
			      size_t len)
{
	struct f2fs_folio_state *ffs;
	unsigned int nr_subpages = ffs_nr_subpages(folio);
				       ...
				       size_t len)
{
	...
	spin_lock_irqsave(&ffs->state_lock, flags);
	bitmap_clear(ffs->state, nr_subpages + start, end - start);
	dirty = find_next_bit(ffs->state, 2 * nr_subpages, nr_subpages) <
		2 * nr_subpages;
	spin_unlock_irqrestore(&ffs->state_lock, flags);
...
int f2fs_encrypt_one_page(struct f2fs_io_info *fio)
{
	...
	if (fio->compressed_page) {
		...
		crypt_folio = page_folio(page);
		crypt_offs = 0;
	} else {
		crypt_folio = fio->folio;
		crypt_offs = ((size_t)fio->idx) << PAGE_SHIFT;
	}
...
retry_encrypt:
	fio->encrypted_page = fscrypt_encrypt_pagecache_blocks(crypt_folio,
					PAGE_SIZE, crypt_offs, gfp_flags);
...
/*
 * Write a range of subpages within a large folio.
 * For each subpage in [start, end), perform IPU or OPU as appropriate.
 * The op lock (cp_rwsem read) is lazily acquired on OPU path and held
 * across the entire folio; caller is responsible for unlock via op_lock_held.
 */
static int f2fs_write_single_data_folio(struct folio *folio, int *submitted,
					struct bio **bio_ptr, sector_t *last_block_ptr,
					struct writeback_control *wbc,
					enum iostat_type io_type, u64 start, u64 end)
{
	struct inode *inode = folio->mapping->host;
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	...
	pgoff_t folio_start_idx;
	pgoff_t folio_end_idx;
	int err = 0;
	int local_submitted = 0;

	folio_start_idx = (start - pos) >> PAGE_SHIFT;
	folio_end_idx = (end - 1 - pos) >> PAGE_SHIFT;
	...
	for (pgoff_t i = folio_start_idx; i <= folio_end_idx; ++i) {
		...
		if (!atomic_commit && need_inplace_update(&fio) &&
			...
			if (!f2fs_is_valid_blkaddr(sbi, fio.old_blkaddr,
						   DATA_GENERIC_ENHANCE)) {
				err = -EFSCORRUPTED;
				goto rollback_this_page;
			}
			ipu_force = true;
			goto got_it;
		}

		/* 2) Normal dnode lookup under the folio-scoped op lock */
...
got_it:
		...
		/* 3) IPU path under the folio-scoped op lock */
...
rollback_this_page:
		...
		if (folio_has_ffs(folio)) {
			if (!ffs)
				ffs = (struct f2fs_folio_state *)folio->private;
			atomic_sub_and_test(1, &ffs->write_pages_pending);
		}
		break;
	}
...
/*
 * Write dirty folios using the folio-based writeback_iter API.
 * Handles large folios with per-subpage dirty tracking via ffs.
 * Used for non-compressed regular files.
 */
				   ...
				   enum iostat_type io_type)
{
	...
	while ((folio = writeback_iter(mapping, wbc, folio, &err))) {
		...
		bool wb_bias_added = false;

		submitted = 0;
		next = true;
		retry = false;
		/* give a priority to WB_SYNC threads */
		if (atomic_read(&F2FS_I_SB(inode)->wb_sync_req[DATA]) &&
...
retry:
		...
		if (!op_lock_held) {
			if (!f2fs_trylock_op(F2FS_I_SB(inode))) {
				folio_redirty_for_writepage(wbc, folio);
				err = 0;
		...
		/*
		 * Normal data writeback must respect i_size; however, while
		 * building fs-verity metadata we also need to write dirty pages
		 * beyond i_size before clearing FI_VERITY_IN_PROGRESS.
		 */
	...
	if (need_balance && !IS_NOQUOTA(inode) &&
		...
		if (folio_test_large(folio)) {
			if (f2fs_is_atomic_file(inode))
				err = prepare_large_folio_atomic_write_begin(inode, mapping,
								      folio, pos, len);
			else
			...
			struct folio *folio, void *fsdata)
{
	...
	if (folio_test_large(folio))
		ffs_mark_subrange_dirty(folio, offset_in_folio(folio, pos), copied);
