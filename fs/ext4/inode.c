...
void ext4_set_inode_mapping_order(struct inode *inode)
{
	...
	min_order = READ_ONCE(EXT4_SB(inode->i_sb)->s_min_folio_order_cap);
	max_order = min_t(unsigned int, EXT4_MAX_PAGECACHE_ORDER(inode),
			  READ_ONCE(EXT4_SB(inode->i_sb)->s_max_folio_order_cap));
