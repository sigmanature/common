...
TRACE_EVENT(f2fs_large_folio_dirty_without_ffs,
	...
	TP_fast_assign(
		...
		__entry->nr_pages	= folio_nr_pages(folio);
		__entry->dirty		= folio_test_dirty(folio);
		__entry->uptodate	= folio_test_uptodate(folio);
	...
	TP_printk("dev=(%d,%d) ino=%lu index=%lu order=%u nr_pages=%u dirty=%d uptodate=%d pid=%d comm=%s",
		show_dev_ino(__entry),
		(unsigned long)__entry->index,
		__entry->order,
		__entry->nr_pages,
...
TRACE_EVENT(f2fs_large_folio_ffs_bootstrap_dirty,
...
TRACE_EVENT(f2fs_large_folio_mkwrite,

	TP_PROTO(struct inode *inode, struct folio *folio, pgoff_t pidx,
		 loff_t pos, loff_t isize, loff_t valid_end, size_t dirty_len,
		 bool need_alloc, bool has_ffs_before, bool dirty_before),

	TP_ARGS(inode, folio, pidx, pos, isize, valid_end, dirty_len,
		need_alloc, has_ffs_before, dirty_before),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(ino_t,	ino)
		__field(pgoff_t, pidx)
		__field(pgoff_t, folio_index)
		__field(unsigned int, order)
		__field(unsigned int, nr_pages)
		__field(size_t, folio_size)
		__field(loff_t, pos)
		__field(loff_t, isize)
		__field(loff_t, valid_end)
		__field(size_t, dirty_len)
		__field(int, need_alloc)
		__field(int, has_ffs_before)
		__field(int, dirty_before)
		__field(pid_t, pid)
		__array(char, comm, TASK_COMM_LEN)
	),
	...
	TP_fast_assign(
		...
		__entry->ino		= inode->i_ino;
		__entry->pidx		= pidx;
		__entry->folio_index	= folio->index;
		__entry->order		= folio_order(folio);
		__entry->nr_pages	= folio_nr_pages(folio);
		__entry->folio_size	= folio_size(folio);
		__entry->pos		= pos;
		__entry->isize		= isize;
		__entry->valid_end	= valid_end;
		__entry->dirty_len	= dirty_len;
		__entry->need_alloc	= need_alloc;
		__entry->has_ffs_before	= has_ffs_before;
		__entry->dirty_before	= dirty_before;
		__entry->pid		= current->pid;
		memcpy(__entry->comm, current->comm, TASK_COMM_LEN);
	),

	TP_printk("dev=(%d,%d) ino=%lu pidx=%lu folio_index=%lu order=%u nr_pages=%u folio_size=%zu pos=%lld isize=%lld valid_end=%lld dirty_len=%zu need_alloc=%d has_ffs_before=%d dirty_before=%d pid=%d comm=%s",
		show_dev_ino(__entry),
		(unsigned long)__entry->pidx,
		...
		__entry->dirty_len,
		__entry->need_alloc,
		__entry->has_ffs_before,
		__entry->dirty_before,
		__entry->pid,
		__entry->comm)
);

TRACE_EVENT(f2fs_large_folio_write_zero_subpage,

	TP_PROTO(struct inode *inode, struct folio *folio, pgoff_t data_idx,
		 pgoff_t sub_idx, u64 start, u64 end, bool has_ffs,
		 bool blk_dirty, bool blk_uptodate),

	TP_ARGS(inode, folio, data_idx, sub_idx, start, end, has_ffs,
		blk_dirty, blk_uptodate),

	TP_STRUCT__entry(
		__field(dev_t,	dev)
		__field(ino_t,	ino)
		__field(pgoff_t, folio_index)
		__field(pgoff_t, data_idx)
		...
		__field(u64, end)
		__field(int, has_ffs)
		__field(int, blk_dirty)
		__field(int, blk_uptodate)
		__field(pid_t, pid)
		__array(char, comm, TASK_COMM_LEN)
	),
	...
	TP_fast_assign(
		...
		__entry->ino		= inode->i_ino;
		__entry->folio_index	= folio->index;
		__entry->data_idx	= data_idx;
		__entry->sub_idx	= sub_idx;
		__entry->order		= folio_order(folio);
		__entry->nr_pages	= folio_nr_pages(folio);
		__entry->start		= start;
		__entry->end		= end;
		__entry->has_ffs	= has_ffs;
		__entry->blk_dirty	= blk_dirty;
		__entry->blk_uptodate	= blk_uptodate;
		__entry->pid		= current->pid;
		memcpy(__entry->comm, current->comm, TASK_COMM_LEN);
	),

	TP_printk("dev=(%d,%d) ino=%lu folio_index=%lu data_idx=%lu sub_idx=%lu order=%u nr_pages=%u start=%llu end=%llu has_ffs=%d blk_dirty=%d blk_uptodate=%d pid=%d comm=%s",
		show_dev_ino(__entry),
		(unsigned long)__entry->folio_index,
		(unsigned long)__entry->data_idx,
		...
		__entry->has_ffs,
		__entry->blk_dirty,
		__entry->blk_uptodate,
		__entry->pid,
		__entry->comm)
);

TRACE_EVENT(f2fs_mapping_set_error,

	TP_PROTO(struct inode *inode, int error, pid_t pid),

	TP_ARGS(inode, error, pid),
...
TRACE_EVENT(f2fs_readpages,
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int,	min_order)
		__field(unsigned int,	max_order)
	),
	...
	TP_fast_assign(
		...
		__entry->min_order = mapping_min_folio_order(inode->i_mapping);
		__entry->max_order = mapping_max_folio_order(inode->i_mapping);
	),

	TP_printk("dev = (%d,%d), ino = %lu, start = %lu nrpage = %u min_order = %u max_order = %u",
		show_dev_ino(__entry),
...
TRACE_EVENT(f2fs_read_large_folio_hole,

	TP_PROTO(struct inode *inode, struct folio *folio, pgoff_t sub_idx,
		 bool blk_uptodate),

	TP_ARGS(inode, folio, sub_idx, blk_uptodate),
	...
	TP_printk("dev=(%d,%d) ino=%lu idx=%lu order=%u sub=%lu uptodate=%d pid=%d comm=%s",
		show_dev_ino(__entry),
		(unsigned long)__entry->folio_index,
		__entry->order,
		(unsigned long)__entry->sub_idx,
...
TRACE_EVENT(f2fs_read_large_folio_mapped,

	TP_PROTO(struct inode *inode, struct folio *folio, pgoff_t offset,
		 unsigned int len_blks),

	TP_ARGS(inode, folio, offset, len_blks),
	...
	TP_fast_assign(
		...
		__entry->order		= folio_order(folio);
		__entry->offset		= offset;
		__entry->len_blks	= len_blks;
		__entry->pid		= current->pid;
		memcpy(__entry->comm, current->comm, TASK_COMM_LEN);
	),

	TP_printk("dev=(%d,%d) ino=%lu idx=%lu order=%u offset=%lu len=%u pid=%d comm=%s",
		show_dev_ino(__entry),
		(unsigned long)__entry->folio_index,
		__entry->order,
