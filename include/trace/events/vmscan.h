...
TRACE_EVENT(mm_vmscan_reclaim_pages,
	...
	TP_STRUCT__entry(
		...
		__array(u32, ordbkt, NR_PAGE_ORDERS)
	),
	...
	TP_fast_assign(
		...
		for (int i = 0; i < NR_PAGE_ORDERS; i++)
			__entry->ordbkt[i] = stat->nr_reclaimed_ordbkt[i];
	),

	TP_printk("nid=%d nr_scanned=%ld nr_reclaimed=%ld nr_dirty=%ld nr_writeback=%ld nr_congested=%ld nr_immediate=%ld nr_activate_anon=%d nr_activate_file=%d nr_ref_keep=%ld nr_unmap_fail=%ld ordbkt=%s",
		__entry->nid,
		...
		__print_array(__entry->ordbkt, NR_PAGE_ORDERS, sizeof(u32)))
);
