...
static void zram_free_page_npages(struct zram *zram, size_t index, int nr_pages);
static int zram_read_from_zspool(struct zram *zram, struct page *page,
				 u32 index, unsigned long page_size);
...
int zram_writeback_slots(struct zram *zram, struct zram_pp_ctl *ctl)
{
	...
	while ((pps = select_pp_slot(ctl))) {
		...
		if (!zram_test_flag(zram, index, ZRAM_PP_SLOT))
			goto next;
		if (zram_read_from_zspool(zram, page, index, PAGE_SIZE))
			goto next;
		zram_slot_unlock(zram, index);
...
{
	struct zram *zram = dev_to_zram(dev);
	unsigned long order_zpdescs = 0;
	ssize_t ret;

	down_read(&zram->init_lock);
	if (init_done(zram))
		order_zpdescs = zs_get_multi_pages_zpdescs(zram->mem_pool);

	ret = sysfs_emit(buf, "order%u_large_pages %lu\n",
			 CONFIG_ZSMALLOC_MULTI_PAGES_ORDER, order_zpdescs);
	up_read(&zram->init_lock);
...
static void zram_free_page_npages(struct zram *zram, size_t index, int nr_pages)
{
	...
	if (zram_test_flag(zram, index, ZRAM_COMP_MULTI_PAGES)) {
		...
		nr_pages = max(nr_pages, ZCOMP_MULTI_PAGES_NR);
	}
#endif
...
static void zram_free_page(struct zram *zram, size_t index)
{
	zram_free_page_npages(zram, index, 1);
}
				 ...
				 u32 index, unsigned long page_size)
{
	if (zram_test_flag(zram, index, ZRAM_SAME) ||
		...
		return read_same_filled_page(zram, page, index, page_size);

	if (!zram_test_flag(zram, index, ZRAM_HUGE))
		return read_compressed_page(zram, page, index, page_size);
	else
		return read_incompressible_page(zram, page, index, page_size);
}

static int zram_read_page_size(struct zram *zram, struct page *page, u32 index,
			       struct bio *parent, unsigned long page_size)
{
	int ret;
	...
	if (!zram_test_flag(zram, index, ZRAM_WB)) {
		...
		ret = zram_read_from_zspool(zram, page, index, page_size);
		zram_slot_unlock(zram, index);
	} else {
		/*
...
static int zram_read_page(struct zram *zram, struct page *page, u32 index,
			  struct bio *parent)
{
	return zram_read_page_size(zram, page, index, parent, PAGE_SIZE);
}
...
static int zram_write_page(struct zram *zram, struct page *page, u32 index)
{
	...
	mem = kmap_local_page(page);
	same_filled = page_same_filled(mem, &element, page_size);
	kunmap_local(mem);
	if (same_filled)
		return write_same_filled_page(zram, element, index, nr_pages);

	zstrm = zcomp_stream_get(zram->comps[ZRAM_PRIMARY_COMP]);
	mem = kmap_local_page(page);
	ret = zcomp_compress(zram->comps[ZRAM_PRIMARY_COMP], zstrm,
			     mem, page_size, &comp_len);
	kunmap_local(mem);
	...
	if (comp_len >= huge_class_size[huge_class_idx]) {
		zcomp_stream_put(zstrm);
		return write_incompressible_page(zram, page, index, page_size,
						 nr_pages);
	}
...
#ifdef CONFIG_ZRAM_MULTI_PAGES
static inline u32 zram_multi_pages_head_index(u32 index)
{
	return index & ~((u32)ZCOMP_MULTI_PAGES_NR - 1);
}

static int __test_multi_pages_comp(struct zram *zram, u32 index)
{
	int i, count = 0;
	u32 head = zram_multi_pages_head_index(index);

	for (i = 0; i < ZCOMP_MULTI_PAGES_NR; i++) {
		if (zram_test_flag(zram, head + i, ZRAM_COMP_MULTI_PAGES))
			count++;
	}
...
static bool want_multi_pages_comp(struct zram *zram, struct bio *bio)
{
	u32 index = bio->bi_iter.bi_sector >> SECTORS_PER_PAGE_SHIFT;
	bool ret;
	...
	zram_slot_lock(zram, index);
	ret = __test_multi_pages_comp(zram, index);
	zram_slot_unlock(zram, index);

	return ret;
}

static bool test_multi_pages_comp(struct zram *zram, struct bio *bio)
{
	u32 index = bio->bi_iter.bi_sector >> SECTORS_PER_PAGE_SHIFT;
	bool ret;
...
static bool is_multi_pages_partial_io(struct bio_vec *bvec)
{
	return bvec->bv_len != ZCOMP_MULTI_PAGES_SIZE;
}

static int zram_bvec_read_multi_pages_partial(struct zram *zram,
					      struct bio_vec *bvec,
					      u32 index, int offset,
					      struct bio *bio)
{
	...
	page = alloc_pages(GFP_NOIO | __GFP_COMP, ZCOMP_MULTI_PAGES_ORDER);
	if (!page)
		return -ENOMEM;

	ret = zram_read_page_size(zram, page, index, bio, ZCOMP_MULTI_PAGES_SIZE);
	if (!ret) {
		dst = kmap_local_page(page);
		memcpy_to_bvec(bvec, dst + offset);
		kunmap_local(dst);
				      ...
				      u32 index, int offset, struct bio *bio)
{
	if (is_multi_pages_partial_io(bvec))
		return zram_bvec_read_multi_pages_partial(zram, bvec, index,
							  offset, bio);

	return zram_read_page_size(zram, bvec->bv_page, index, bio,
				   ZCOMP_MULTI_PAGES_SIZE);
}
...
{
	if (is_multi_pages_partial_io(bvec))
		return zram_bvec_write_multi_pages_partial(zram, bvec, index,
							   offset, bio);
...
static void zram_bio_read_multi_pages(struct zram *zram, struct bio *bio)
{
	...
	do {
		u32 index = zram_multi_pages_head_index(iter.bi_sector >>
							SECTORS_PER_PAGE_SHIFT);
		u32 offset = (iter.bi_sector & (SECTORS_PER_MULTI_PAGE - 1)) <<
			     SECTOR_SHIFT;
		struct bio_vec bv = multi_pages_bio_iter_iovec(bio, iter);

		atomic64_inc(&zram->stats.zram_bio_read_multi_pages_count);
		bv.bv_len = min_t(u32, bv.bv_len,
				  ZCOMP_MULTI_PAGES_SIZE - offset);
...
static void zram_bio_write_multi_pages(struct zram *zram, struct bio *bio)
{
	...
	do {
		...
		atomic64_inc(&zram->stats.zram_bio_write_multi_pages_count);
		bv.bv_len = min_t(u32, bv.bv_len,
				  ZCOMP_MULTI_PAGES_SIZE - offset);
...
#ifdef CONFIG_ZRAM_MULTI_PAGES
	{
		...
		if (comp_count > 1) {
			...
			if (comp_count == 1) {
				zram_clear_flag(zram, index, ZRAM_COMP_MULTI_PAGES);
				zram_slot_unlock(zram, index);
				index = zram_multi_pages_head_index(index);
				nr_pages_to_free = ZCOMP_MULTI_PAGES_NR;
				if (!zram_slot_trylock(zram, index)) {
...
#ifdef CONFIG_ZRAM_MULTI_PAGES
	zram_free_page_npages(zram, index, nr_pages_to_free);
#else
	zram_free_page(zram, index);
#endif
	zram_slot_unlock(zram, index);
