		...
		struct file_ra_state *ra)
{
	...
	new_order = max(new_order, readahead_min_order);
...
static ssize_t readahead_min_order_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%u\n", readahead_min_order);
}

static ssize_t readahead_min_order_store(struct kobject *kobj,
						 ...
						 const char *buf, size_t count)
{
	...
	if (ret)
		return ret;
	if (val > MAX_PAGECACHE_ORDER)
		return -EINVAL;
	readahead_min_order = val;
	return count;
}

static struct kobj_attribute readahead_min_order_attr =
	__ATTR(min_order, 0644, readahead_min_order_show, readahead_min_order_store);

static struct attribute *readahead_attrs[] = {
	&readahead_min_order_attr.attr,
	NULL,
};

static struct attribute_group readahead_attr_group = {
	.attrs = readahead_attrs,
};

static int __init readahead_init(void)
{
	struct kobject *ra_kobj;
	int ret;

	ra_kobj = kobject_create_and_add("readahead", mm_kobj);
	if (!ra_kobj)
		return -ENOMEM;

	ret = sysfs_create_group(ra_kobj, &readahead_attr_group);
	if (ret)
		kobject_put(ra_kobj);
